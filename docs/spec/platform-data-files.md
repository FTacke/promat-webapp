...
- Public access requests use the canonical `/access-request` page, store one request record in the auth/core database, and on valid submission attempt one server-side operator notification through the configured mail transport instead of exposing a public `mailto` draft as the active journey.
