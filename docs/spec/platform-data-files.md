...
- A public Reading page may provide localized top-level `footnotes_html`. Footnote references in safe editorial HTML target stable footnote IDs, and the shared renderer places the compact footnote section immediately before a final `pm-literature` section when present, otherwise after the normal sections. Footnotes use safe editorial HTML for inline emphasis, remain smaller than main prose, and do not merge into the literature list.
