...
- PROMAT login uses email address for personal accounts. Group accounts log in with a `login_name` (stored in the `username` column); this is a narrowly scoped exception and does not introduce public username login for ordinary personal users. Public self-registration is not part of the active product contract. See `docs/spec/auth-accounts.md` for the full account-type contract.
...
- The productive protected-area role model contains only `user` and `admin`; `editor` is not part of the active PROMAT product contract. A separate `account_kind` dimension (`personal` | `group`) classifies accounts by type without changing roles; group accounts always carry `role = user`. See `docs/spec/auth-accounts.md`.
...
- Admin user management uses the canonical `/admin/users` route family for personal account creation, status updates, optional expiry dates, invitation/reset preparation, and explicit admin-triggered invitation/reset email sending. Group accounts are created and managed via the `/admin/groups` route family; they use direct password-setting without invitation email. Opening the prepared invitation dialog must not send mail. The manual copy fallback for link, subject, and body remains available even when direct sending is disabled or fails.
...
- The canonical protected default targets after login are: safe requested target first, otherwise `/auth/account` for personal `user` accounts, the research home page for group accounts, and `/admin/users/page` for `admin`.
...
- On Teaching routes, the mobile current-area block reuses the same local navigation structure as Research but shows only the current Teaching context. The Teaching root shows the single active `Sprachauswahl` / `Language selection` entry. Teaching language hubs and topic pages show the current teaching language as a non-clickable group label and only one active child link, `Themenseiten` / `Topic pages`, pointing to that language hub; topic titles and alternative Teaching languages are not listed in the drawer. This mobile Teaching context does not introduce a desktop Teaching sidebar.
...
- Shared public `reading` pages may attach ordered `content_elements` to a normal prose section. The productive expandable material type is `pm_expandable_text` with one stable technical `id`, localized `title` and `summary_html`, and a complete ordered `items` list of `label` plus `text` values; it renders after that section's prose rather than as a detached page-level work area.
- A public Reading expandable is progressively enhanced: without JavaScript its complete material remains readable, while the active client initially shows the localized title, summary, and an approximately three-line faded preview. Its shared toggle uses the localized `Anzeigen`/`Ausblenden` and `Show`/`Hide` states, synchronizes `aria-expanded` with its controlled region, and reveals every source item when expanded.
- Expandable Reading material remains typographically subordinate to main prose and uses the shared surface, border, radius, spacing, action-button, and reading-type tokens. Its item list uses two compact columns on desktop, one column on narrow screens, quiet tabular labels, and avoids splitting individual entries where the browser supports it.
- A public Reading page may provide localized top-level `footnotes_html`. Footnote references in safe editorial HTML target stable UI-language-suffixed footnote IDs, and every rendered footnote provides a localized accessible return link to its first stable reference. The shared renderer places the compact footnote section immediately before a final `pm-literature` section when present, otherwise after the normal sections. Footnotes use safe editorial HTML for inline emphasis, remain smaller than main prose, span the same Reading measure as main prose and literature without a narrower inner column, and do not merge into the literature list. The global navigation scroll handler must preserve valid fragment targets on click, direct deep-link load, hash change, and browser history traversal instead of applying the normal page-navigation reset to the top.
- The shared `pm-literature` list remains the canonical hanging-indent literature treatment on Reading pages and uses a restrained scale below main prose without inheriting expandable or footnote styling.
...
- On that research section root, each corpus card shows only the localized corpus title, then the primary metadata order `Projektleitung`/`Project lead`, `Materialkonzeption`/`Material design`, `Durchführung`/`Conducted by`, and then the secondary status order learner-recordings count or `Korpus im Aufbau`/`Corpus in progress` followed by an optional reference-recordings line that counts distinct native-speaker/reference-speaker `person_id` values rather than `standard_variety` values.
...
- The public corpus landing page `/{ui_lang}/research/{corpus_language}` is a reduced orientation page. On desktop with the left sidebar visible, the main column must not repeat `Design`, `Sprecher:innen`, `Vergleich`, or `Phänomene` as a second list, card set, or CTA wall.
- On breakpoints where the left sidebar is hidden, that same public corpus landing page shows one short orientation sentence and a compact link-pill block for `Design`, `Sprecher:innen`, `Vergleich`, and `Phänomene` directly in the main column. The pill block must reuse the same generic research-navigation order, labels, route targets, and auth/lock metadata as the sidebar.
- In the desktop main column of that public corpus landing page, the visible structure is limited to the localized corpus title, one short subtitle, two short prose paragraphs, and for signed-out users one small action row with exactly two actions in this order: `Zugang beantragen`/`Request access`, then `Zum Login`/`Go to login`.
...
- The shared footer legal links keep the same technical slugs in both UI languages: German routes point to `/de/impressum` and `/de/privacy`, English routes point to `/en/impressum` and `/en/privacy`. Visible labels are `Impressum` / `Datenschutz` in German and `Legal Notice` / `Privacy` in English.
...
- Productive `speakers` and `comparison` workbench headers use the direct localized page title without an additional explanatory intro line; orientation belongs to the workbench controls, states, and empty-state copy rather than a duplicate header subtitle.
...
- The canonical French wordlist value for `wl_014` is `théâtre`. French intake normalizes the historical misspelling without the circumflex before label validation and runtime artifact generation, and reports each applied correction as `canonical_item_correction`.
...
- Wordlist and text TextGrid readers must tolerate the active exported encodings used by intake tools, including UTF-16 and UTF-8 with or without BOM, without requiring manual re-encoding of original transcript files.
...
- Docker-backed `text` MFA uses one shared model cache per intake language under `scripts/research_data_intake/.mfa_cache/shared/{language_code}/` for MFA acoustic and dictionary models. The shared model cache is not person-scoped, not manifest-hash-scoped, and is separate from batch-local `working/{person_id}/text/mfa_output/` alignment outputs.
- Before Docker-backed alignment, the runner must check for the configured acoustic model and dictionary files in that shared cache and run `mfa model download` only for missing models. The subsequent `mfa align` command must stay separate from the model-download step.
...
- Missing `db/import_payload.json` does not block file publish by itself: release stage and `current` switch may proceed for runtime files, while DB upsert remains a separate explicit workflow.
- A packaged `db/import_payload.json` must not be applied to the production DB by default. The server-side publish path may run the DB upsert only when an explicit flag such as `--apply-db-upsert` is set.
- The production DB payload upsert contract is limited to `research_people`, `research_sessions`, and `research_session_exposures`. It must validate the staged release before writing, reject invalid payloads and local Windows paths, run a dry-run plan first, then apply in one transaction before the atomic `current` switch.
- The DB payload upsert is keyed by stable `person_id`, `session_id`, and `(session_id, sort_order)` exposure rows. Re-running the same payload must not create duplicate people, sessions, or exposure rows. Payload sessions may replace only their own ordered exposure rows; data for sessions outside the payload must not be deleted by omission.
- A session is publishable only if at least one of its tasks (wordlist, text, or interview) has actual delivered working data or existing runtime artifacts. Sessions that appear only in the intake workbook without any working audio or task artifacts (metadata-only sessions) must not be written to the runtime filesystem, must not appear in `db/import_payload.json`, and must not be upserted into the production DB. `validate_payload_against_release` rejects payloads that include sessions with empty `documented_tasks`.
- Metadata-only sessions that reach the production DB or runtime through an earlier import (before this rule was enforced) can be cleaned up via `apply_prod_db_payload.py --cleanup-metadata-only --target-language <code> --release-dir <release>` (dry-run by default; `--apply-cleanup` to apply). Cleanup is scoped strictly to the specified target language; it removes session rows, their exposure rows, and person rows that have no remaining publishable sessions. Cleanup must not touch other languages, must not create backups or snapshots, and must always be verified via a dry-run before applying.
...
- Package validation rejects the historical noncanonical spelling of French `wl_014` in `sessions/french/**/*.json` and `config/research_player/french/**/*.json`, so a stale generated export cannot reintroduce it during a later publish.
...
- The initial v0.7 production server model is data-only: prod upload delivery targets `/srv/webapps_storage/promat/data/incoming/{upload_id}/` first, validates allowlist paths plus checksums plus file counts, stages a new release under `data/releases/{release_id}/`, promotes via an atomic relative `data/current` symlink switch, and then rsyncs each corpus from the new release into the flat `data/sessions/{corpus_slug}/` tree that the app reads directly.
- In that v0.7 server model, application data is mounted from `/app/data` with `PROMAT_RUNTIME_ROOT=/app`; `get_sessions_root()` resolves to `/app/data/sessions` (the flat corpus tree), not via `current`. The `current` symlink points to the latest release using a relative path (`releases/{release_id}`) and serves as a rollback reference and DB-upsert input path, not as the app's primary session-file read path.
