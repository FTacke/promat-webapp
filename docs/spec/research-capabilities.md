...
- `phenomena` is productive for a corpus when the canonical task catalogs (`wordlist` and `text`) load successfully through the shared research-player config layer and the PostgreSQL research-set storage is available.
- The curated sets shown in the `phenomena` overview are stored exclusively in the PostgreSQL `research_sets` table (visibility = `curated`); an empty curated-set list is a valid DB state and yields a plain empty productive overview rather than a config failure.
- `phenomena_presets.json` files under `data/config/research_player/{language}/` are **legacy seed files** and are no longer used at runtime; they are not part of the productive `phenomena` capability check.
