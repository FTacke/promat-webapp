...
- Diese Marker bleiben im Working-JSON nicht roh stehen: `segment.text` und `tokens[].text` enthalten nur den gesprochenen Text, während `annotations[]` strukturierte `material_ref`-Einträge mit `item_id`, `task`, `label`, `item_number`, `canonical_text`, `insert_after_token_id` und optional `trailing_punctuation` tragen.
